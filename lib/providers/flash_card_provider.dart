...
class FlashCardProvider extends ChangeNotifier {
  ...
  FlashCard? get currentCard =>
      _sessionCards.isNotEmpty ? _sessionCards[_currentCardIndex] : null;
  ...
  int get cardsRemaining => _currentSession?.isEndless == true
      ? -1
      : (_currentSession != null
            ? _currentSession!.totalCards - _getSessionProgress()
            : 0);
  int get sessionProgress => _currentSession?.reviewedCards.length ?? 0;
  int get sessionTotal => _currentSession?.isEndless == true
      ? -1
      : (_currentSession?.totalCards ?? 0);
  ...
  Future<void> _loadFlashCards() async {
    try {
      ...
      if (storedCards != null && storedCards.isNotEmpty) {
        final Map<String, dynamic> decodedCards =
            jsonDecode(storedCards) as Map<String, dynamic>;
        _cards = decodedCards.map(
          (key, value) =>
  ...
  Future<void> _loadSessions() async {
    try {
      ...
      if (storedSessions != null && storedSessions.isNotEmpty) {
        final List<dynamic> decodedSessions =
            jsonDecode(storedSessions) as List<dynamic>;
  ...
  Future<void> _saveFlashCards() async {
    try {
      ...
      final cardsJson = jsonEncode(
        _cards.map((key, value) => MapEntry(key, value.toJson())),
      );
  ...
  Future<bool> startSession({
    ...
    try {
      ...
      if (allCards.length < numberOfCards && !isEndless) {
        print(
          'Warning: Requested $numberOfCards cards but only ${allCards.length} available. Cards will be repeated as needed.',
        );
      }
      ...
      allCards.sort((a, b) {
        ...
        final levelComparison = levelOrder[a.level]!.compareTo(
          levelOrder[b.level]!,
        );
      ...
      final actualSessionSize = isEndless
          ? min(100, allCards.length)
          : max(1, numberOfCards);
  ...
  Future<void> markCard(bool isCorrect) async {
    ...
    if (_currentCardIndex >= _sessionCards.length) {
      print(
        'Error: Current card index out of bounds: $_currentCardIndex >= ${_sessionCards.length}',
      );
    ...
    if (_currentCardIndex >= _sessionCards.length) {
      ...
      _sessionCards.shuffle();
      print(
        'Cycled back to beginning of card list with ${_sessionCards.length} cards',
      );
    }
  ...
  Map<String, dynamic> getStatsForWordList(String wordListId) {
    ...
    final difficultCards = cards
        .where((card) => card.level == CardLevel.difficult)
        .length;
    final learningCards = cards
        .where((card) => card.level == CardLevel.learning)
        .length;
    final masteredCards = cards
        .where((card) => card.level == CardLevel.mastered)
        .length;
    ...
    final averageAccuracy = cardsWithReviews > 0
        ? totalAccuracy / cardsWithReviews
        : 0.0;
  ...
  Map<String, dynamic> getOverallStats() {
    ...
    final difficultCards = _cards.values
        .where((card) => card.level == CardLevel.difficult)
        .length;
    final learningCards = _cards.values
        .where((card) => card.level == CardLevel.learning)
        .length;
