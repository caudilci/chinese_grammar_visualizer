...
List<DictionaryEntry> _chineseSearch(
  List<DictionaryEntry> entries,
  String query,
) {
...
/// Search pinyin
List<DictionaryEntry> _pinyinSearch(
  List<DictionaryEntry> entries,
  String query,
) {
    ...
    results = entries
        .where(
          (entry) =>
              ...
              PinyinUtils.toNumericalPinyin(
                entry.pinyin,
              ).contains(queryNumerical),
        )
        .toList();
  } else {
  ...
  if (results.isEmpty && query.length >= 2 && !query.contains(' ')) {
    ...
    results = entries.where((entry) {
      final entrySyllables = PinyinUtils.getPlainPinyin(
        entry.pinyin,
      ).split(' ');
...
List<DictionaryEntry> _englishSearch(
  List<DictionaryEntry> entries,
  String query,
) {
  ...
  if (queryWords.length > 1) {
    ...
    results = entries
        .where(
          (entry) => entry.definitions.any(
            (def) => def.toLowerCase().contains(normalizedQuery),
          ),
        )
        .toList();
  }
  ...
  final exactMatches = results
      .where(
        (entry) => entry.definitions.any(
          (def) => def.toLowerCase() == normalizedQuery,
        ),
      )
      .toList();
  ...
  if (exactMatches.isNotEmpty) {
    ...
    final nonDuplicates = results
        .where((e) => !exactMatches.contains(e))
        .toList();
...
void _sortDefinitionResultsByRelevance(
  ...
  results.sort((a, b) {
    ...
    final List<String> aLowerDefs = a.definitions
        .map((def) => def.toLowerCase())
        .toList();
    final List<String> bLowerDefs = b.definitions
        .map((def) => def.toLowerCase())
        .toList();
    ...
    final aExactWordMatchCount = _countRegexMatches(
      aDefinitions,
      exactWordRegExp,
    );
    final bExactWordMatchCount = _countRegexMatches(
      bDefinitions,
      exactWordRegExp,
    );
